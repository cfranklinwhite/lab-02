...
author: "Team Name: Charlie Franklin-White, Ming Jiang, Sean, Duncan"
date: "`r Sys.Date()`"
...
Africa is the region with the most coastal countries
...
plastic_data_all %>%
  group_by(region) %>%
  summarise(average_mismanaged = mean(total_mismanage_plastic))

plastic_data_all %>%
  group_by(region) %>%
  summarise(total_per_region = sum(total_mismanage_plastic)) %>%
  arrange(desc(total_per_region))


```

Asia is the region with the highest total and Oceania is the region with the lowest total.
...
mutate(pct_mismanaged_plastic_ocean = (mismanaged_plastic_ocean * population) / total_mismanage_plastic) %>%
  group_by(region) %>%
  summarise(median_of_pct = median(total_mismanage_plastic, na.rm = TRUE)) %>%
  filter(code == "SOM") 
